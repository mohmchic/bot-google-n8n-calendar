...
export const generatePromptSeller = (history: string, message: string) => {
...
const flowSeller = addKeyword(EVENTS.ACTION)
    .addAction(async (_, { state, flowDynamic, extensions }) => {
        try {
            ...
            const promptInfo = generatePromptSeller(history, lastMessage.content)
            console.log(promptInfo)

            const response = await ai.assistant(promptInfo)
